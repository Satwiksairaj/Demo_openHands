    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "owner": {"type": "string"},
                "repo": {"type": "string"},
                "path": {"type": "string", "description": "Path in repo (default: root)"},
...
async def handle_jira_add_comment(args: dict) -> str:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post(
            url, auth=(JIRA_EMAIL, JIRA_TOKEN), json=body
        )
        resp.raise_for_status()
...
async def handle_jira_update_status(args: dict) -> str:
    ...
    if not transition_id:
        ...
        return json.dumps({"error": f"Status '{status_name}' not found. Available: {available}"})
...
async def handle_github_list_files(args: dict) -> str:
    ...
    return json.dumps([
        {"name": i["name"], "type": i["type"], "path": i["path"], "size": i.get("size", 0)}
        for i in items
    ])
